...
import { Users2Icon, Clock, Code2 } from 'lucide-react';
...
const ServerPage: React.FC = () => {
  const serverInfo = {
    ...
    features: [
      { title: 'Nhiệm vụ', icon: <Code2 size={19} />, desc: '100+ nhiệm vụ' },
      {
        title: 'Kinh tế',
        icon: <Users2Icon size={19} />,
        desc: 'Thị trường tự do',
      },
      {
        title: 'Cộng đồng',
        icon: (
        ...
        desc: 'Thân thiện, hỗ trợ',
      },
      { title: 'Cập nhật', icon: <Clock size={19} />, desc: 'Liên tục' },
    ],
    stats: {
      players: '1000+',
      uptime: '99.9%',
    ...
    <div className="min-h-screen font-sans text-white">
      ...
      <div className="mb-8 flex justify-center gap-8">
        <div className="flex items-center gap-2 rounded border-l-4 border-[#00fff7] bg-black/20 px-4 py-2 shadow">
          <Users2Icon size={19} className="text-[#00fff7]" />
        ...
        <div className="flex items-center gap-2 rounded border-l-4 border-[#ff00cc] bg-black/20 px-4 py-2 shadow">
          <Clock size={19} className="text-[#ff00cc]" />
        ...
        <div className="flex items-center gap-2 rounded border-l-4 border-[#00bfff] bg-black/20 px-4 py-2 shadow">
          <Code2 size={19} className="text-[#00bfff]" />
          ...
          {serverImages.map((img, idx) => (
            <div key={idx} className="group relative flex-1">
              <img
                src={img.url}
                alt={img.title}
                className="h-48 w-full object-cover opacity-20 transition-transform duration-300 group-hover:scale-105 group-hover:opacity-50"
                style={{
                  borderRight:
        ...
        <div className="flex flex-wrap justify-center gap-4">
          {serverInfo.features.map((f, idx) => (
            ...
            >
              ...
              <span className="font-simibold text-xs">
                {f.title.toUpperCase()}
              </span>
